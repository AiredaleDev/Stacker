...
void stacker_program_init(Program* program) {
    program->len = 0;
...
void stacker_program_deinit(Program* program) {
...
void stacker_program_clear(Program* program) {
    program->len = 0;
    // FIXME: This fucker frees memory that readline uses??? 
    memset(program->instructions, 0, program->cap * sizeof(StackerInst));
}

void stacker_program_append_instruction(Program* program, StackerInst inst) {
    if (program->len == program->cap) {
        stacker_program_grow(program);
    }
    ++program->len;
...
void stacker_program_append_instructions(Program* program, StackerInst* insts, usize nemb) {
    ...
    while (program->cap < program->len + nemb) {
        stacker_program_grow(program); // Arguably we should just allocate all the size upfront.
    }
    
    for (usize i = 0; i < nemb; ++i) {
        stacker_program_append_instruction(program, insts[i]);
    }
}
